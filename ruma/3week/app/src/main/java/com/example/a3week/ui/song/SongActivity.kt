...
import com.example.a3week.ui.main.album.CustomSnackbar
...
class SongActivity : AppCompatActivity() {
    ...
    private fun moveSong(direct: Int){
        if (nowPos + direct < 0){
            CustomSnackbar.Companion.make(binding.root, "처음 곡입니다.").show()
            // Toast.makeText(this,"처음 곡입니다.",Toast.LENGTH_SHORT).show()
            return
        }

        if (nowPos + direct >= songs.size){
            CustomSnackbar.Companion.make(binding.root, "마지막 곡입니다").show()
    ...
    private fun setPlayer(song: Song){
        ...
        if (cover != null && cover != 0) {
            binding.songAlbumIv.setImageResource(cover)
        } else {
            binding.songAlbumIv.setImageResource(R.drawable.img_album_exp4)
        }
